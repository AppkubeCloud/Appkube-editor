...
const DASHBOARD_NAMES = ["overall-s3-development", "overall-s3-test", "overall-s3-stage", "overall-s3-production"];

const PARAMETERS_NAMES = ["sla", "performance", "reliability", "availability", "endUsage", "security", "cost", "dataProtection"];

class OverallS3Explorer extends Component<Record<string, string>, LocalState> {
  popupRef: any;
  constructor(props: Record<string, string>) {
    ...
    this.state = {
      ...
      value: 0,
      dashboardIDs: {
        "overall-s3-development": "",
        "overall-s3-test": "",
        "overall-s3-stage": "",
        "overall-s3-production": ""
      },
      selectedParam: "sla"
    };
    ...
    return (
      <div className="overall-explorers-container">
        <div className="heading">
          <h1><img src={images.pageTitleIcon} alt="" /> S3 (Simple Storage Service)</h1>
        ...
        <div className="tabs-container">
          ...
          <div className="tabs-contents">
            {value === 0 ? (
              <Development dashId={dashboardIDs[`overall-s3-development-${selectedParam}`]} />
            ) : value === 1 ? (
              <Test dashId={dashboardIDs[`overall-s3-test-${selectedParam}`]} />
            ) : value === 2 ? (
              <Stage dashId={dashboardIDs[`overall-s3-stage-${selectedParam}`]} />
            ) : value === 3 ? (
              <Production dashId={dashboardIDs[`overall-s3-production-${selectedParam}`]} />
            ) : (
              <></>
            )}
          </div>
...
export default OverallS3Explorer;
